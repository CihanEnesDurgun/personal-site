 ...
 * @param {Object} [context] - Optional context (endpoint, method, user) for richer error detail
...
const validateRequired = (data, requiredFields, context = {}) => {
    ...
    if (missing.length > 0) {
        const where = context && context.endpoint ? ` (${context.method || ''} ${context.endpoint})`.trimEnd() : '';
        throw new AppError('VAL-2002', null, `Missing fields: ${missing.join(', ')}${where}`);
    }
};
